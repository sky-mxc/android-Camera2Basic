...
public class Camera2BasicFragment extends Fragment
    ...
    private void setUpCameraOutputs(int width, int height) {
        ...
        try {
            for (String cameraId : manager.getCameraIdList()) {
                ...
                //这里的图片尺寸 是最大的，格式也是不是 推荐的camera2格式而是camera的格式
                mImageReader = ImageReader.newInstance(largest.getWidth(), largest.getHeight(),
                        ImageFormat.JPEG, /*maxImages*/1);
                mImageReader.setOnImageAvailableListener(
                        mOnImageAvailableListener, mBackgroundHandler);

                //找出我们是否需要交换尺寸以获得相对于传感器坐标的预览大小
                // Find out if we need to swap dimension to get the preview size relative to sensor
                // coordinate.
                /**
                 * 拿到手机的旋转角度
                 * The returned value may be
                 * {@link Surface#ROTATION_0 Surface.ROTATION_0 (no rotation),
    ...
    private void startBackgroundThread() {
        Log.e("startBackgroundThread", "handler");
    ...
    private void stopBackgroundThread() {
        Log.e("stopBackgroundThread","camera2");
    ...
    private  class ImageSaver implements Runnable {
        ...
        public void run() {
            ...
            Log.e("ImageSaver","buffer->"+buffer.remaining());
